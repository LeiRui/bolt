...
// Dictionary(LazyVector): indirect lazy must use SimpleCallableHook (align
// SimpleNumericAggregate::updateGroups), even when numNulls_==0 after batch 1.
// Primary regression on Linux aarch64 + 256-bit SVE (LAZY hook before SVE);
// on other hosts SubOp delegates to Base and this still checks parity.
TEST_F(SumAggregationTest, sumInt64SubOpIndirectLazyMatchesBase) {
#if defined(_WIN32)
  GTEST_SKIP() << "BOLT_SPARK_SUM_INT64_USE_SUBOP uses POSIX setenv/unsetenv";
#else
  auto lazyValues = makeLazyFlatVector<int64_t>(
      5, [](vector_size_t row) { return static_cast<int64_t>((row + 1) * 10); });
  const std::vector<RowVectorPtr> batches = {
      makeRowVector({
          makeFlatVector<int32_t>({0, 1, 2}),
      ...
      makeRowVector({
          makeFlatVector<int32_t>({0, 0, 1, 1, 2}),
          wrapInDictionary(makeIndices({0, 1, 2, 3, 4}), 5, lazyValues),
      }),
  };
